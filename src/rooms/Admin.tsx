...
function PostComposer({ onPost }: { onPost: () => void }) {
  ...
  const [category, setCategory] = useState<'art' | 'music' | 'tech' | 'personal' | ''>('')
    ...
    >
      ...
      {!isStory && (
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(['art', 'music', 'tech', 'personal'] as const).map(c => (
            <button
              key={c}
              type="button"
...
function PostList({ refreshKey }: { refreshKey: number }) {
  ...
  // Cycle a post's category: (none) → art → music → tech → personal → (none).
  const cycleCategory = async (post: any) => {
    const order = [null, 'art', 'music', 'tech', 'personal'] as const
